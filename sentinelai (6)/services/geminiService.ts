...
// Debug helper for local development issues
if (!apiKey) {
    console.warn("WARNING: SentinelAI API Key is missing. Detection features will fail locally.");
}
...
export const askSecurityAdvisor = async (query: string): Promise<string> => {
    ...
    try {
        const response = await ai.models.generateContent({
            ...
            contents: query,
            config: {
                systemInstruction: "You are an elite Cybersecurity Consultant for SentinelAI. Your creators are MOHAMED NIZAMUDDIN A, YASHWANT K, and THILAK RAJ . P. Provide concise, immediate, and actionable security advice using NLP. Keep responses under 200 words and focus on speed and accuracy. Use markdown formatting.",
            }
        });
...
export const askSiteGuide = async (query: string): Promise<string> => {
  ...
  try {
      const response = await ai.models.generateContent({
          ...
          contents: query,
          config: {
              systemInstruction: "You are SentinelBot, a friendly and helpful AI assistant for SentinelAI. Your creators are MOHAMED NIZAMUDDIN A, YASHWANT K, and THILAK RAJ . P. Your job is to guide users on how to use the website (Deep Scan, Awareness, Advisor) AND answer general cybersecurity questions (like 'what is phishing?'). Keep responses very short, conversational, and helpful.",
          }
      });
      return response.text || "I'm having trouble connecting to the mainframe.";
  } catch (e) {
      console.error(e);
